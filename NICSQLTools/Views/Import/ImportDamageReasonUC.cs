...
{
    public partial class ImportDamageReasonUC : DevExpress.XtraEditors.XtraUserControl
    {

        #region -   Variables   -
        private static readonly ILog Logger = log4net.LogManager.GetLogger(typeof(ImportDamageReasonUC));
...
", Environment.NewLine);
        ...
        private bool ImportFromExcel()
        {
            ...
            foreach (DataRow row in dtExcel.Rows)
            {
                ...
                NICSQLTools.Data.dsData.DMG_ReasonRow SqlRow = dsData.DMG_Reason.NewDMG_ReasonRow();
                //Remove duplicated
                if (dsData.DMG_Reason.FindByBilling_Document(row["Billing Document"].ToString()) != null)
                    continue;
                ...
                dsData.DMG_Reason.AddDMG_ReasonRow(SqlRow);
                SqlRow.EndEdit();
            }

            ShowHideProgress(true); ChangeProgressCaption("Updating Damage Reason ...");

            if (!DMG_Reason.UpdateBulkDMG_Reason(cmd, dsData.DMG_Reason))
                MsgDlg.Show("Update Damage Reason Failed", MsgDlg.MessageType.Error);
            else
            {
                AddLog("New Damage Reason Saved " + dsData.DMG_Reason.Count);
                output = true;
            }
            dsData.DMG_Reason.AcceptChanges();
            ShowHideProgress(false);
            dtExcel.Rows.Clear(); dtExcel.Dispose(); dtExcel = null;
            dsData.DMG_Reason.Clear(); dsData.DMG_Reason.Dispose();
